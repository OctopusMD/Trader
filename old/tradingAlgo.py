...
from old.getStockData import csv_to_dataset
...
if __name__ == "__main__":
    ...
    ohlcv_histories, technical_indicators, next_day_open_values, unscaled_y, y_normaliser, dates = csv_to_dataset(
        '../csv/MSFT_daily.csv', False)
    ...
    #dates_test = plt.dates.date2num(dates_test)

    real = plt.plot(dates_test[1:], unscaled_y_test[start:end].tolist(), label='real')
    pred = plt.plot(dates_test[1:], y_test_predicted[start:end].tolist(), label='predicted')
